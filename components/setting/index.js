...
const Aa = ({ user, userInfo1 }) => {
  const router = useRouter();
  const useer = JSON.parse(user);
  const [bio, setBio] = useState();
  const [email, setEmail] = useState();
  const [username, setUsername] = useState();
  const [poosts, setPoosts] = useState(useer.posts);
  ...
  async function handleDeletePost(e, postId) {
    ...
    try {
      ...
      if (data.sign == true) {
        alert("Success");
        const data = poosts.filter((p) => p._id !== postId);
        setPoosts(data);
      } else {
        alert("Error");
      }
  ...
  async function handleSubmit(event) {
    ...
    const body = {
      ...
      email,
      username,
      _id: userInfo1.userId,
    };
    if (!bio || !email || !userInfo1.userId) {
        ...
        <div className="flex flex-col items-center ">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col w-3/4 mt-3">
            ...
            <input
              ...
              className="p-3 rounded-md bg-zinc-700 border-0  mb-3 text-white"
            />{" "}
            <button type="submit" className="bg-pink-900 p-3 w-1/4 rounded-md">
              Submit !
            </button>
            {error && <span className="mb-3 text-red-600">{error}</span>}
          </form>

          <section className="w-3/4 mt-3">
                            ...
                            <div className="grid grid-cols-3 justify-items-end">
                              <div className="text-md flex flex-col items-center">
                                ...
                                <span className="text-sm">
                                  <TimeAgo datetime={p.date} locale="fa" />
                                </span>
                              </div>
                              <div className="text-md flex flex-col items-center">
                                <span>
